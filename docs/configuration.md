...
| `MAX_CHALLENGE_LENGTH` | `2048` | Maximum login, consent, and logout challenge length in bytes. `0` uses the default; negative values or values exceeding `4096` are invalid. The value must fit within the HTTP server's 16 KB request-header limit. |
