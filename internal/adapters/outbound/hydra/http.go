...
import (
	...
	hydraapi "github.com/ory/hydra-client-go/v2"
...
func (c *Client) GetLoginRequest(ctx context.Context, challenge string) (domain.LoginRequest, error) {
	...
	return domain.LoginRequest{
		...
		RequestedAAL: requestedAAL(response.OidcContext),
	}, nil
}
...
func requestedAAL(context *hydraapi.OAuth2ConsentRequestOpenIDConnectContext) string {
	if context == nil || len(context.AcrValues) == 0 {
		return ""
	}
	return context.AcrValues[0]
}
